...
#     @ seconds_begin = $seconds_end - $time_between_perturbations
     ...
     #${COPY} ${RUNDIR}/rttov_sensor_db.csv ${RUNDIR}/${dt}
     #${COPY} ${RUNDIR}/rttov_sensor_db.csv ${RUNDIR}/advance_temp1
     sleep 1
     #srun -n 1 --mpi=pmi2 ${RUNDIR}/WRF_RUN/grid_refl_obs ${RUNDIR}/${dt}/obs_seq.final.${dt} $refl_min $days_begin $seconds_begin $days_end $seconds_end $innov wrfinput_d01     
     #srun -n 1 --mpi=pmi2 python ${PYDIR}/grid_refl_obs.py -o ${RUNDIR}/OBSOUT/obs_seq.nc.${dt} --refl_min $refl_min --time $days_begin $seconds_begin $days_end $seconds_end --innov $innov -f wrfinput_d01 -m wrf  --fortran
     srun -n 1 python ${PYDIR}/grid_refl_obs.py -o ${RUNDIR}/OBSOUT/obs_seq.nc.${dt} --refl_min $refl_min --time $days_begin $seconds_begin $days_end $seconds_end --innov $innov -f wrfinput_d01 -m wrf  --fortran
   ...
   if ($perturbations_needed == 1) then
     ...
     @ transition_relative_time = 86400 * ($wrfdays - $transition_time_days) + ($wrfsecs - $transition_time_secs)
      ...
      if ($transition_relative_time <= 0) then
	   ...
	   #srun -n 1 --mpi=pmi2 ${RUNDIR}/WRF_RUN/add_pert_where_high_refl ../refl_obs_${wrfsecs}_${wrfdays}.txt wrfinput_d01 $lh $lv $u_noise_1 $v_noise_1 $w_noise_1 $t_noise_1 $td_noise_1 $qv_noise_1 $wrfdays $wrfsecs $ensemble_member
           mpirun -np 1 ${RUNDIR}/WRF_RUN/add_pert_where_high_refl ../refl_obs_${wrfsecs}_${wrfdays}.txt wrfinput_d01 $lh $lv $u_noise_1 $v_noise_1 $w_noise_1 $t_noise_1 $td_noise_1 $qv_noise_1 $wrfdays $wrfsecs $ensemble_member
       else
        echo "(2) member ${ensemble_member} calling add_pert_where_high_refl"
	   #srun -n 1 --mpi=pmi2 ${RUNDIR}/WRF_RUN/add_pert_where_high_refl ../refl_obs_${wrfsecs}_${wrfdays}.txt wrfinput_d01 $lh $lv $u_noise_2 $v_noise_2 $w_noise_2 $t_noise_2 $td_noise_2 $qv_noise_2 $wrfdays $wrfsecs $ensemble_member
	   mpirun -np 1 ${RUNDIR}/WRF_RUN/add_pert_where_high_refl ../refl_obs_${wrfsecs}_${wrfdays}.txt wrfinput_d01 $lh $lv $u_noise_2 $v_noise_2 $w_noise_2 $t_noise_2 $td_noise_2 $qv_noise_2 $wrfdays $wrfsecs $ensemble_member
       endif
