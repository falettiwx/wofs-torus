...
if ( -e ${RUNDIR}/add_noise_py.csh ) then
   set USE_NOISE = 1
else
   set USE_NOISE = 0
endif
...
while($state_copy <= $num_states)
   ...
   #### USE non updated file
   cp $temp_dir/wrfinput_d01_orig $temp_dir/wrfinput_d01

   #### UPDATE FRP AND SMOKE IN WRFINPUT FILE
   #   if ( ${CHEM} == 18 ) then
	   #  cp ${RUNDIR}/prep_chem_sources.inp .

     # GATHER WILDFIRE INFO INTO FILE
     #     srun -n 1 ${EXEDIR}/prep_chem_sources.exe
     #sleep 2
     #ln -sf ./FIRE-HRRR_new-T-${startyear}-${startmonth}-${startday}-${starthour}${startmin}00-g1-bb.bin  ./emissfire_d01
     #ln -sf ./FIRE-HRRR_new-T-${startyear}-${startmonth}-${startday}-${starthour}0000-g1-bb.bin  ./emissfire_d01
         
     # ADD WILDFIRE INFO TO wrfinput FILE
     #srun -n 1 python ${EXEDIR}/prepchem2wrf.py -i emissfire_d01 -w wrfinput_d01
     #sleep 2
     #endif

   ### SAVE pre-noise wrfinput for use to initiate forecasts
   ...
   set ADV_MOD_COMMAND = "srun ${RUNDIR}/WRF_RUN/wrf.exe"

   # radar additive noise option.  if shell script is available
   # in the centraldir, it will be called here.
   if ( $USE_NOISE ) then
      ${RUNDIR}/add_noise_py.csh $wrfsecs $wrfdays $state_copy $ensemble_member $temp_dir $RUNDIR $datea
   endif
   ...
   #while ( $wrfkey < $targkey )
      ...
      #echo $infl | srun -n 1  ${RUNDIR}/WRF_RUN/update_wrf_bc >&! out.update_wrf_bc
...
# The EOF on the line above MUST REMAIN in column 1.
      ...
      sed -f advModel.sed ${NAME_DIR}/namelist.input.member${ensemble_member} >! namelist.input
