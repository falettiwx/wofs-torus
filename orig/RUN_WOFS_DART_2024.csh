...
while ( 1 == 1 )
    ...
    if ( ${datea} == ${runDay}2215 ) then # MAKE THE RUN START TIME A VARIABLE!! (4/23/2024)
    ...
    if ( ${datea} == ${startdate} ) then
       cp ${TEMPLATE_DIR}/input.nml.init ${RUNDIR}/input.nml
    else
       cp ${TEMPLATE_DIR}/input.nml.cycle ${RUNDIR}/input.nml 
    endif
	...
	set startyear  = `echo ${datea} | cut -c1-4`
        ...
        ${LINK} /scratch/wof-torus/obs/obs_seq_RF_${startyear}${startmonth}${startday}_${starthour}${startmin}.out obs_seq.out  
