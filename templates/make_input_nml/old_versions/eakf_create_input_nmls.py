...
inlier_threshold = 0 # exclude obs less than this many standard deviations from the mean
...
nml_names = ['init', 'cycle', f'cycle.{exclude_ext}'] # nml file name extensions
inf_initials = ['false', 'true', 'true'] # set to false in init nml only
inf_sd_initials = ['false', 'true', 'true'] # set to false in init nml only
w_state_bounds = ['', "'W','-50.0','60.0','CLAMP',", "'W','-50.0','60.0','CLAMP',"] # W not included in init nml
vardictlist = [vardicts, vardicts, vardicts_incl] # List of vardicts to include in each respective nml
...
for i in range(len(nml_names)):
    ...
    outstr = return_nml_str(varlist=varlist,
                            ...
                            inf_initial=inf_initials[i], 
                            inf_sd_initial = inf_sd_initials[i], 
